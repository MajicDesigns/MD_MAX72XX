...
void MD_MAX72XX::clear(addressType startDev, addressType endDev)
{
  if (endDev < startDev) return;

  for (addressType buf = startDev; buf <= endDev; buf++)
  {
...
bool MD_MAX72XX::getBuffer(addressType col, addressType size, uint8_t *pd)
{
...
bool MD_MAX72XX::setBuffer(addressType col, addressType size, uint8_t *pd)
{
...
bool MD_MAX72XX::getPoint(uint8_t r, addressType c)
{
...
bool MD_MAX72XX::setPoint(uint8_t r, addressType c, bool state)
{
  uint8_t buf = c/COL_SIZE;
  c %= COL_SIZE;
...
bool MD_MAX72XX::setRow(addressType startDev, addressType endDev, uint8_t r, uint8_t value)
{
  bool b = _updateEnabled;
...
bool MD_MAX72XX::transform(addressType startDev, addressType endDev, transformType_t ttype)
{
 // uint8_t t[ROW_SIZE];
  uint8_t colData;
  bool b = _updateEnabled;
  ...
  switch (ttype)
  {
    ...
    for (long long int buf = endDev; buf >= (long long int)startDev; --buf)
    {
    ...
    for (addressType buf = 0; buf < (endDev - startDev + 1)/2; buf++)
    {
      deviceInfo_t	t;
